...
def image_interrogate(
    ...
    print(f"Number of tags before filtering: {len(result[1])}")
    ...
    print(f"Postprocessed tags: {postprocessed_tags}")
...
if args.file:
    ...
    print(f"Tags with confidence >= 0.85: {high_confidence_tags}")

    # Create a filename with the current date and time
    current_time = datetime.now().strftime("%Y%m%d%H%M%S")
    txt_filename = Path(f"{current_time}.txt")

    # Write the high confidence tag names to a text file
    with open(txt_filename, "w") as txt_file:
        txt_file.write(", ".join(high_confidence_tags))
